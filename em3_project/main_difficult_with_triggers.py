...
def setup_eeg_trigger(port_name="COM4", baudrate=115200):
    """Create a trigger sender. Falls back to mock-print if serial is unavailable."""
    try:
        eeg_port = serial.Serial(port_name, baudrate)
        print(f"EEG trigger port ready: {port_name}")

        def send_trigger(code):
            eeg_port.write(int(code).to_bytes(1, 'big'))
            print(f"trigger sent {code}")

        return send_trigger, eeg_port
    except Exception as exc:
        print(f"EEG trigger port not set ({exc}). Using mock triggers.")
...
# ─── MemoryTrial ──────────────────────────────────────────────────────────────
def MemoryTrial(tree, prob_tree, entropy_tree, pitch_tree, altposition, send_triggers=True):
    path_tones     = [tree[0]]
...
# ─── ProductionTrial ──────────────────────────────────────────────────────────
def ProductionTrial(tree, prob_tree, entropy_tree, pitch_tree, altposition, send_triggers=True):
    path_tones     = [tree[0]]
    path_probs     = [prob_tree[0]]
    path_entropy   = [entropy_tree[0]]
    path_pitch_dif = [pitch_tree[0]]
...
# ─── TestTrial ────────────────────────────────────────────────────────────────
def TestTrial(seq, change, pos, col, generated, surprisal, send_triggers=True):
    win.color = C_PAGE
...
clock = core.Clock()
trigger, port = setup_eeg_trigger("/dev/tty.usbserial-DN2Q03LO", 115200)
