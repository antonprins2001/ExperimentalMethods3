...
SIMULATE = False  # Set True to run without audio/manual input (uses sequences_test.csv, subject ID 999)
...
def CollectTrials(trial_seqs, subject_id):
    ...
    pd.DataFrame(columns=list(trial_data.keys())).to_csv(trial_file, index=False)
    pd.DataFrame(columns=list(test_data.keys())).to_csv(test_file, index=False)
    ...
    for trial_num, seq_data in enumerate(trial_seqs):
        ...
        pd.DataFrame({k: v[-8:] for k, v in trial_data.items()}).to_csv(trial_file, mode='a', header=False, index=False)
        pd.DataFrame({k: [v[-1]] for k, v in test_data.items()}).to_csv(test_file, mode='a', header=False, index=False)

    return test_data, trial_data

path = "Sequence/sequences_test.csv" if SIMULATE else "Sequence/sequences_new.csv"
